...
export default function Dashboard() {
  ...
  // Cheques por vencer (próximos 15 días)
  const hoy = new Date()
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        backgroundImage: 'radial-gradient(circle at 90% 50%, rgba(201,168,76,0.08), transparent 60%)'
      }}>
        <div style={{ fontFamily: "'Bebas Neue', cursive", fontSize: 32, letterSpacing: 2, color: 'var(--gold)' }}>
          {saludo}, {nombre} 👋
        </div>
        ...
        <div style={{ fontSize: 13, color: 'var(--muted)', marginTop: 8 }}>
          Sistema de gestión · Carnicerías Fabricius · Río Primero, Córdoba
        </div>
      </div>

      {/* ALERTAS */}
      {(chequesPorVencer.length > 0 || clientesDeuda.length > 0) && (
            ...
            <div style={{ background: '#2a1a0a', border: '1px solid var(--amber)', borderRadius: 10, padding: '10px 16px', flex: 1, minWidth: 200 }}>
              ...
              {chequesPorVencer.map(ch => (
                <div key={ch.id} style={{ fontSize: 12, color: 'var(--text2)' }}>
                  #{ch.numero} — {ch.cliente_nombre} — {fmtFull(ch.monto)} — vence {ch.fecha_pago}
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
          {[
            ...
            <div key={s.label} style={{ background: 'var(--surface2)', borderRadius: 10, padding: 16, textAlign: 'center' }}>
              ...
              <div style={{ fontFamily: "'Bebas Neue', cursive", fontSize: 32, color: s.kg < 100 ? 'var(--red-light)' : s.color }}>
                {s.kg.toFixed(1)} kg
              </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
        ...
        {/* ÚLTIMOS GASTOS + ACCESOS RÁPIDOS */}
        <div>
          ...
          {[
            ...
            <div key={s.nombre}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 8 }}>
                ...
                <span style={{ color: s.color, fontFamily: "'Bebas Neue', cursive", fontSize: 20 }}>
                  {fmt(totMesGanancia * s.pct / 100)}
                </span>
              </div>
              <div style={{ background: 'var(--border)', borderRadius: 4, height: 10, borderRadius: 8 }}>
