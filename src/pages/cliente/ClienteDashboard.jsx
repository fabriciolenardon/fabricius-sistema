...
function useCliente() {
  ...
  useEffect(() => {
    ...
    cargar()
    // Suscripción a cambios de saldo (refresco automático cuando admin registra movimientos)
    const canal = supabase.channel('cliente-saldo')
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'clientes', filter: `id=eq.${profile.cliente_id}` }, (payload) => {
  ...
  async function cargar() {
    setLoading(true)
    const { data } = await supabase.from('clientes').select('*').eq('id', profile.cliente_id).maybeSingle()
    setCliente(data)
    setLoading(false)
  }
...
export function ClienteDashboard() {
  ...
  useEffect(() => {
    if (!cliente) return
    cargarDatos()
    const canal = supabase.channel('remitos-cliente')
...
export function ClienteCtaCte() {
  ...
  return (
    <div>
      ...
      <div className="card">
        <div className="card-title">Historial completo ({movimientos.length} movimientos)</div>
        <table>
          <thead><tr><th>Fecha</th><th>Tipo</th><th>Descripción</th><th>Debe</th><th>Haber</th><th>Saldo</th></tr></thead>
          <tbody>
            {pag.items.map(m => (
...
export function ClienteRemitos() {
  ...
  return (
    <div>
      ...
      <div className="page-sub">Historial completo de despachos recibidos</div>
      <div className="card">
